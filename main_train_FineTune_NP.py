    ...
    for ite_epoch in range(epoch_step2):
        ...
        for ite_stack in range(num_TrainingStack):
            ...
            for ite_batch in range(num_batch):
                ...
                print("\rstep 2 - epoch %2d/%2d - training stack %2d/%2d - batch %3d/%3d" % \
                    (ite_epoch+1, epoch_step2, ite_stack+1, num_TrainingStack, ite_batch+1, num_batch), end="")
                ...
                if ((ite_batch + 1) == int(num_batch / 2)) or ((ite_batch + 1) == num_batch):
                    ...
                    print("\rstep 2 - epoch %2d - imp PSNR: %.3f - ori PSNR: %.3f - MSE loss of MC: %.5f - MSE loss of QE: %.8f" % \
        ...
        for ite_stack in range(num_ValidationStack):
            ...
            for ite_batch in range(num_batch):
                ...
                print("\rstep 2 - epoch %2d/%2d - validation stack %2d/%2d                " % \
                    (ite_epoch + 1 + int(res_index) + 1, epoch_step2 + int(res_index) + 1, ite_stack+1, num_ValidationStack), end="")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ri', '--res_index', type=str, help="index of model for restoration (start from 0).")
    parser.add_argument('-es2', '--epoch_step2', type=int)
    ...
    dir_model_pre = "./"
    dir_model = dir_model_pre + "model_QP" + QP
    model_res_path = os.path.join(dir_model, "model_step2.ckpt-" + res_index)
    
    record_FileName = dir_model_pre + "record_train_QP" + QP + ".txt"
    file_object = open(record_FileName, 'a')
